package com.huanzi.design_pattern.chapter2_wait_Set;

/**
 * 1.所有的对象都会有一个wait set，用来存放调用了 该对象wait()方法之后进入block状态线程
 * 2.线程被notify()之后不一定立即得到执行
 * 3.线程从wait set中被  唤醒的顺序(随机唤醒) 不一定是FIFO，根据JVM的实现来决定
 * 4.线程执行wait()方法之后，
 * 		会释放所持有的锁，被唤醒之后再一次获取锁，
 * 		之后从执行wait()方法后的代码继续执行
 */
	...
	private static void work(){
		synchronized (LOCK){
			...
			try {
				...
				LOCK.wait();  //放入到wetSet,进入blocking状态
